...
The idiom for this repo (a TypeScript package) is below.

## TypeScript / SDK conventions

This is a published, widely-imported package, so the bar is high.

### File headers

A module header is a comment block at the top of the file, above imports and exports.
Follow the shared header guidance above (describe the abstraction and the load-bearing
invariant, not a list of contents). Exported types and functions carry their own JSDoc;
the header stays high-level. `src/index.ts` is the reference example.

### Linting and types

- `pnpm run typecheck` (`tsc --noEmit`) must pass. The tsconfig is strict, including
  `noUncheckedIndexedAccess` and `exactOptionalPropertyTypes`.
- `pnpm run lint` (ESLint, typescript-eslint strict-type-checked) must pass with zero
  warnings. The public API carries explicit input and output types.

`pnpm run build` bundles with esbuild and emits declarations with `tsc`.
